#include "../periodic_drtf_definition.h"
#include "Halide.h"
...
class PeriodicDRTf_generator
   : public Halide::Generator<PeriodicDRTf_generator> {
 private:
   PeriodicDRTf q;
   Func cin;
   Func q_out{"q_out"};
 ...
 public:
   ...
   void generate() {
      ...
      // clang-format on
      q.periodic_drtf(clamped_in, q_out, x, y, c);
      // clang-format off
   ...
   void schedule() {
      if (auto_schedule) {
         ...
         out.set_estimate(i, 0, VAL_N * 4)
            .set_estimate(j, 0, VAL_N);
      } else {
...
HALIDE_REGISTER_GENERATOR(PeriodicDRTf_generator, periodic_drtf)
