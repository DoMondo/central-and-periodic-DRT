...
class periodic_drt2output_generator
   : public Halide::Generator<periodic_drt2output_generator> {
 ...
 public:
   ...
   void generate() {
      ...
      Expr outN = out.width() / 4.0f;
      Expr xfactor = 4.0f * N / out.width();
      Expr yfactor = N / out.height();
      ...
      Expr ydir = clamp(cast<uint16_t>(y * yfactor), 0, N);
...
HALIDE_REGISTER_GENERATOR(periodic_drt2output_generator, periodic_drt2output)
